...
import { fetchTopRatedMovies, setTopRatedPage } from "../../redux/slices/moviesSlice";
...
export default function TopRatedMovies() {
  ...
  const { topRated, topRatedPage, totalTopRatedPages, topRatedLoading, genres } = useSelector(state => state.movies);
  ...
  }, [dispatch, topRatedPage, selectedGenre, selectedSort]);

  return (
    <div className="bg-[#1F2A37] min-h-screen">
      <div className="pt-20 container mx-auto px-4">
        <h2 className="text-4xl font-bold text-white px-4">Top Rated Movies</h2>
        ...
        <section className="grid grid-cols-6 gap-5">
          {topRatedLoading ? (
            ...
            topRated?.slice(0, 24).map(movie => <Card key={movie.id} data={movie} />)
          )}
        </section>
        ...
        <div className="mt-6">
          <Pagination
            ...
            onPageChange={(page) => dispatch(setTopRatedPage(page))}
          />
        </div>
      </div>
    </div>
